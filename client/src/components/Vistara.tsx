...
export function Footer() {
  return <footer className="bg-black text-[var(--gesso)]">
    <div className="container grid gap-12 py-16 md:grid-cols-[1.2fr_1fr_1fr] md:py-20">
      ...
      <div><p className="mono text-[var(--oak)]">Services</p><div className="mt-5 flex flex-col gap-3 text-[14px] text-[var(--gesso)]/75"><Link href="/home-interiors-bangalore">Residential interiors</Link><Link href="/home-interiors-bangalore">Modular kitchens</Link><Link href="/commercial-interior-designers-bangalore">Commercial & corporate</Link><Link href="/home-interiors-bangalore">Construction & architecture</Link><Link href="/home-interiors-bangalore">Vastu-aligned planning</Link></div></div>
      <div><p className="mono text-[var(--oak)]">Studio</p><div className="mt-5 flex flex-col gap-3 text-[14px] text-[var(--gesso)]/75"><a href="/#projects" onClick={(e) => { const el = document.getElementById("projects"); if (el) { e.preventDefault(); el.scrollIntoView({ behavior: "smooth" }); } }}>Projects</a><a href="/" onClick={(e) => { if (window.location.pathname === "/") { e.preventDefault(); window.scrollTo({ top: 0, behavior: "smooth" }); } }}>About Vistara</a><Link href="/contact">Contact</Link></div><p className="mt-10 text-[12px] text-[var(--gesso)]/45">© 2026 Vistara Interior. All rights reserved.</p></div>
    </div>
  </footer>;
}
...
export function ProjectCard({ project, index = 0 }: { project: Project; index?: number }) {
  return <article className={`reveal group ${index % 2 === 1 ? "md:translate-y-10" : ""}`} style={{ animationDelay: `${index * 80}ms` }}>
    ...
    <div className="project-meta mt-4"><div className="flex items-start justify-between gap-4"><h3 className="display text-[29px] leading-none">{project.title}</h3></div><p className="mono mt-4 text-[10px] text-[var(--muted-ink)]">{project.spec}</p><div className="mt-3 flex flex-wrap gap-1.5">{(project.chips || ["Oak veneer", "Verde marble", "Brushed brass", "Made in our factory"]).map((chip) => <span key={chip} className={`chip ${chip === "Made in our factory" ? "factory" : ""}`}>{chip}</span>)}</div></div>
  </article>;
}
...
export function ServicePage({ kind }: { kind: "residential" | "commercial" }) {
  ...
  const data = residential ? {
    ...
    projects: [{ title: "HSR Office", spec: "HSR OFFICE · TURNKEY · 2025", image: images.hsr, alt: "corporate office reception with oak acoustic wall — office, HSR" }, { title: "Koramangala F&B", spec: "KORAMANGALA F&B · TURNKEY · 2025", image: images.koramangala, alt: "restaurant interior with oak banquette and brushed brass lighting — F&B, Koramangala" }, { title: "MG Road Retail", spec: "MG ROAD RETAIL · TURNKEY · 2025", image: images.mgroad, alt: "boutique retail interior with oak display wall — retail, MG Road" }],
  ...
  return <div className={`page-shell ${residential ? "service-residential" : "service-commercial"}`}><Header /><main><section className="relative overflow-hidden"><div className="absolute inset-0"><img src={data.image} alt={data.alt} className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/50 to-transparent" /></div><div className="container relative z-10 pb-24 pt-28 md:pb-32 md:pt-36"><div className="max-w-[620px]"><p className="eyebrow text-[var(--oak)]">{data.eyebrow}</p><h1 className="display mt-5 text-[clamp(48px,6.5vw,88px)] text-[var(--gesso)]">{data.h1}</h1><p className="mt-7 max-w-[520px] text-[17px] leading-7 text-[var(--gesso)]/75">{data.promise}</p><div className="mt-8 flex flex-wrap items-center gap-5"><Link href="/contact" className="button-fern">Book a consultation <ArrowUpRight size={15} /></Link><WhatsAppLink className="button-fern cursor-pointer"><MessageCircle size={15} />WhatsApp us</WhatsAppLink></div><div className="mt-8 flex flex-wrap gap-x-5 gap-y-2 border-t border-[var(--gesso)]/20 pt-4"><span className="mono text-[var(--oak)]">{residential ? "VILLA HOMES" : "CORPORATE FIT-OUT"}</span><span className="mono text-[var(--gesso)]/60">{residential ? "FINE APARTMENTS" : "HP · TATA · HCL · ORACLE · GTS · ITC"}</span></div></div></div></section><section className="section-pad bg-[var(--limewash)]"><div className="container grid gap-12 md:grid-cols-[.75fr_1.25fr]"><div><p className="eyebrow">The scope</p><h2 className="display mt-4 max-w-[370px] text-[clamp(42px,5vw,70px)]">What's included</h2></div><div className="grid gap-x-10 sm:grid-cols-2">{data.items.map((item, i) => <div key={item} className="oak-rule flex min-h-[88px] items-start gap-5 py-5"><span className="mono text-[var(--brass)]">0{i + 1}</span><p className="text-[16px] leading-6">{item}</p></div>)}</div></div></section><section className="section-pad"><div className="container"><div className="mb-12 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow">Selected work</p><h2 className="display mt-4 text-[clamp(44px,6vw,80px)]">Featured projects</h2></div><Link href="/contact" className="text-[13px] font-semibold text-[var(--fern)] underline decoration-[var(--oak)] underline-offset-4">Discuss your project <ArrowUpRight size={14} className="inline" /></Link></div><div className="grid gap-10 md:grid-cols-3">{data.projects.map((project, i) => <ProjectCard key={project.title} project={project} index={i} />)}</div></div></section><section className="bg-[var(--fern-deep)] py-20 text-[var(--gesso)] md:py-28"><div className="container grid gap-10 md:grid-cols-[.7fr_1.3fr] md:items-center"><p className="eyebrow text-[var(--oak)]">{residential ? "THE VISTARA DIFFERENCE" : "EXECUTION, STRUCTURED"}</p><h2 className="display max-w-[850px] text-[clamp(38px,5.4vw,76px)]">{data.differentiator}</h2></div></section><section className="section-pad"><div className="container grid gap-12 md:grid-cols-[.7fr_1.3fr]"><div><p className="eyebrow">Questions, answered</p><h2 className="display mt-4 text-[clamp(44px,5vw,72px)]">FAQ</h2></div><FAQ items={data.faq} /></div></section><CTA /></main><Footer /></div>;
}
